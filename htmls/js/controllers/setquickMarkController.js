...
app.controller('quickMarkCountCtrl', function($rootScope, $scope, $modal, toastr) {
	//$scope.screenwd=screen.width;
	$scope.screenwd = document.body.clientWidth;
	$scope.isBigScreen = false;
	if($scope.screenwd < 1600) {
		$scope.isBigScreen = false;
	} else {
			...
			if($scope.result.ret == 'success') {
				...
				for(var i = 0; i < $scope.markInfoslist.length; i++) {
					...
					if($scope.isBigScreen) {
						legenditem = $scope.markInfoslist[i].program;
					} else {
						if($scope.markInfoslist.length > 2) {
							if($scope.markInfoslist[i].program.length > 10) {
								legenditem = $scope.markInfoslist[i].program.slice(0, 5) + '...';
							} else {
								legenditem = $scope.markInfoslist[i].program;
							}
						} else {
					...
					$scope.titleList.push(legenditem);
						/*
